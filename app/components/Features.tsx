...
export function Features({
  ...
  return (
    <section className="w-full" style={{ backgroundColor: bgColor, backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'scroll', backgroundRepeat: 'no-repeat' }}>
      <div className="max-w-6xl mx-auto py-14 md:p-20">
        <div className="p-3 text-center" style={{ color }}>
          <h2 className="text-4xl font-bold mb-6 features-title" style={{ color }}>{title}</h2>
...
const Feature = ({
  ...
  return (
    <div className="rounded-lg overflow-hidden">
      ...
      <button
        ...
        onMouseEnter={() => {
          if (window.innerWidth >= 768) {
            onMouseEnter(); // only hover effect on desktop
          }
        }}
        onClick={() => {
          if (window.innerWidth < 768) {
            onToggle(); // only mobile accordion
          }
        }}
      >
        <span
      ...
      {/* MOBILE COLLAPSIBLE */}
      <div
        className={`md:hidden transition-all duration-300 ${isOpen ? "max-h-96 p-4" : "max-h-0 overflow-hidden"
          }`}
      >
